...
def startBatch() {
    sh 'curl ${domain}/download/dims-batch-gx-@project.version@.jar -o dims-batch-gx-@project.version@.jar'
    sh 'java -jar -Xms1G -Xmx4G -Dspring.datasource.username=${template_username} -Dspring.datasource.password=${template_password} -Dspring.datasource.url="jdbc:postgresql://localhost:5432/${template_database}" -Ddims.url="${domain}" dims-batch-gx-@project.version@.jar --job=dataCollector --province=${province} --specialty=${speciality}'
}

def startImport() {
    sh 'curl ${domain}/download/dims-data-issue-importer-@project.version@.jar -o dims-data-issue-importer-@project.version@.jar'
//    sh 'java -jar -Xms1G -Xmx4G -Dspring.datasource.username=${template_username} -Dspring.datasource.password=${template_password} -Dspring.datasource.url="jdbc:postgresql://localhost:5432/${template_database}" dims-data-issue-importer-@project.version@.jar --province=${province} --specialty=${speciality}'
